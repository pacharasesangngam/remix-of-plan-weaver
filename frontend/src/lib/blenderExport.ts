...
const findBestWall = (
  ...
  for (const wall of walls) {
    ...
    const tolerance = Math.max(getWallThicknessM(wall, pw, ph), 0.2);
...
const addWallMeshes = (
  ...
  const geometries = buildWallSolidGeometries(walls.map(wall => {
  ...
  walls.forEach((wall, index) => {
    ...
    wallGroup.userData = {
      ...
      length: wallLengthM, thickness, wallHeight,
      scgPaintCode: wall.scgPaintCode,
      wallFinish: wall.wallFinish,
      wallTexture: wall.wallTexture,
    };
    group.add(wallGroup);
    const mesh = new THREE.Mesh(geometries.get(wall.id)!, material);
    mesh.name = `${wallGroup.name} Solid`;
...
export const exportFloorPlanGlb = async ({
  ...
  const defaultWallHeight =
    rooms.length > 0
      ? Math.max(...rooms.map((room) => safeNum(room.wallHeight, wallHeight)), wallHeight)
      : wallHeight;
