...
const addWallMeshes = (
  ...
  walls.forEach((wall, index) => {
    ...
    const material = new THREE.MeshStandardMaterial({
      color: wall.wallColor ?? paint.hex,
      roughness: 0.72,
      metalness: 0.03,
    });
    ...
    solids.forEach((seg, segIndex) => {
      ...
      addBox(
        ...
        [seg.tStart + segLen / 2 - wallLengthM / 2, seg.yStart + segH / 2, 0],
        material,
      );
    });
  });
...
export const exportFloorPlanGlb = async ({
  ...
  addWallMeshes(scene, walls, doors, windows, defaultWallHeight, pw, ph);
  addDoorMeshes(scene, doors, walls, defaultWallHeight, pw, ph);
  addWindowMeshes(scene, windows, walls, defaultWallHeight, pw, ph);
